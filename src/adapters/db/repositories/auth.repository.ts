...
import type { IAuthRepository } from '@domain/auth/ports';
import { and, eq } from 'drizzle-orm';
import type { NodePgDatabase } from 'drizzle-orm/node-postgres';
import type * as schema from '../client/schema';
import { auth } from '../client/schema';

type DbAuth = typeof auth.$inferSelect;

function toAuthCredential(row: DbAuth): IAuthCredential {
  return {
    id: row.id,
    userId: row.userId,
    email: row.email,
...
export class DrizzleAuthRepository implements IAuthRepository {
  ...
  async findByEmail(email: string): Promise<IAuthCredential | null> {
    const [row] = await this.db.select().from(auth).where(eq(auth.email, email)).limit(1);
    return row ? toAuthCredential(row) : null;
  }

  async findByUserId(userId: string): Promise<IAuthCredential | null> {
    const [row] = await this.db.select().from(auth).where(eq(auth.userId, userId)).limit(1);
    return row ? toAuthCredential(row) : null;
  }

  async findByProvider(provider: string, providerId: string): Promise<IAuthCredential | null> {
  ...
  async updatePasswordHash(userId: string, newHash: string): Promise<void> {
    await this.db.update(auth).set({ passwordHash: newHash }).where(eq(auth.userId, userId));
  }
}
